...
class InputDir:
    ...
    def copy_file(self, src_file: str) -> str:
        with open(src_file, "rb") as f:
            nm = hash_file(f)
            dst = self.path / nm
            dst_remote = self.remote_path / nm
    ...
    def copy_corpus(self, src_dir: str) -> list[str]:
        ...
        for file in src_path.iterdir():
            ...
            if self.copy_corpus_max_size is not None and size > self.copy_corpus_max_size:
                ...
                continue
            files.append(self.copy_file(str(file)))
        return files
    ...
    @classmethod
    def hash_corpus(cls, path: str | Path) -> list[str]:
        path = Path(path)
        hashed_files = []
        for file in path.iterdir():
            # If the file is already a hash, skip it
            if cls.has_hashed_name(file.name):
                continue
            try:
                with open(file, "rb") as f:
                    hash_filename = hash_file(f)
                file.rename(path / hash_filename)
                hashed_files.append(hash_filename)
            except OSError as e:
                # Likely already hashed by another pod
                logger.info(f"Error hashing file: {file.name} {e}")
                continue
        return hashed_files
    ...
    def list_local_corpus(self) -> list[str]:
        return [str(self.path / f.name) for f in self.path.iterdir()]

    def list_remote_corpus(self) -> list[str]:
        return [str(self.remote_path / f.name) for f in self.remote_path.iterdir()]
...
class CrashDir:
    def __init__(self, wdir: str, task_id: str, harness_name: str, count_limit: int | None = None):
        self.wdir = wdir
        self.crash_dir = Path(task_id) / f"{CRASH_DIR_NAME}_{harness_name}"
        self.count_limit = count_limit

    def _input_dir_for_token(self, token: str, sanitizer: str | None = None) -> InputDir:
        token_hash = hashlib.sha256(token.encode()).hexdigest()
        input_dir = self.crash_dir / token_hash
        if sanitizer:
            input_dir = input_dir / sanitizer
        return InputDir(self.wdir, str(input_dir))
...
class Corpus(InputDir):
    def __init__(self, wdir: str, task_id: str, harness_name: str, copy_corpus_max_size: int | None = None):
        ...
        self.corpus_dir = str(Path(task_id) / f"{CORPUS_DIR_NAME}_{harness_name}")
    ...
    def remove_any_merged(self, redis: Redis) -> None:
        ...
        removed = 0
        local_files = {Path(fl).name for fl in self.list_local_corpus()}
